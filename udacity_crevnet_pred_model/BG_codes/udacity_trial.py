...
parser.add_argument('--HMB_NUM', type=int, default=1, help='ID of dataset')
...
dataset = UdacityDataset_LSTM(dataset_path, [f'HMB{opt.HMB_NUM}'], #['HMB1', 'HMB2', 'HMB4', 'HMB5','HMB6'],
                             composed,
...
# --------- optimizer ------------------------------------------
frame_predictor_optimizer = torch.optim.Adam(frame_predictor.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
encoder_optimizer = torch.optim.Adam(encoder.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
...
for e in trange(N_EPOCH):
    for step, (image, steer) in enumerate(train_generator):
        ...
        # print("MAX:", batch_x.max(), "MIN:", batch_x.min())
        if step % 10 == 0:
    ...
    with torch.no_grad():
        ...
        for i in range(len(x_gt_list)):
            ...
            plt.imshow(x_gt_list[i][0][:3].permute(1, 2, 0).cpu().numpy()/10.0+0.5)
            plt.subplot(2, len(x_pred_list), i+1 + len(x_pred_list))
            plt.imshow(x_pred_list[i][0][:3].permute(1, 2, 0).cpu().numpy()/10.0+0.5)
            # plt.imshow(x_gt_list[i][0][:3].permute(1, 2, 0).cpu().numpy()+0.5)
            # plt.subplot(2, len(x_pred_list), i+1 + len(x_pred_list))
            # plt.imshow(x_pred_list[i][0][:3].permute(1, 2, 0).cpu().numpy()+0.5)
        #plt.savefig("BG_tmp_3/test_{}.png".format(e))
        plt.savefig(f"BG_tmp_HMB_{opt.HMB_NUM}/test_{e}.png")

    

torch.save({"frame_predictor": frame_predictor.state_dict(),
            "encoder": encoder.state_dict(),
            "frame_predictor_optimizer": frame_predictor_optimizer.state_dict(),
            "encoder_optimizer": encoder_optimizer.state_dict(),
            "epoch": 0}, f"BG_tmp_HMB_{opt.HMB_NUM}/model_dicts.pt")
