...
class CITY(object):
    def __init__(self, train, data_root, seq_len=13,input_len = 10, gap = 1):
        ...
        for i in self.city:
            ...
            folder = self.data_root+i+'/'+self.data_type
    ...
    def get_sequence(self):
        ...
        while True:
            if self.train:
                ...
                folder = self.data_root + self.c + '/' + self.data_type + '/'
                files = self.flist[c_idx]
                s_idx = np.random.randint(len(files))
                f = files[s_idx]
                #self.data = np.load(folder + f)
                self.data = h5py.File(folder + f, 'r')["array"][:, :, :, :3]
                #print(self.data['array'])
                #print(0, self.data.shape, t)
