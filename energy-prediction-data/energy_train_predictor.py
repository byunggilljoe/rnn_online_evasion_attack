...
#         self.fc2 = torch.nn.Linear(150, 27)
...
lstmp.train()
optimizer = torch.optim.Adam(lstmp.parameters(), lr=0.001)
for e in range(1000):
    lstmp.train()
    for i, (data, target) in enumerate(train_loader):
        ...
        loss.backward()
        optimizer.step()
    # Half prediction test
    
    lstmp.eval()
    with torch.no_grad():
        data_predicted = data[:, :25]
        hidden_and_cell = lstmp.get_init_hidden_cell(data.size(0))
        PREDICTION_STEP = 5
        for i in range(PREDICTION_STEP):
            next_frame, hidden_and_cell = lstmp.get_one_pred(data_predicted, hidden_and_cell)
            data_predicted = torch.cat([data_predicted, next_frame[:, -1:, :]], axis=1)
        # print()
            
        
    
    print(loss.item())
torch.save(lstmp.state_dict(), "tmp/energy_predictor.pt")