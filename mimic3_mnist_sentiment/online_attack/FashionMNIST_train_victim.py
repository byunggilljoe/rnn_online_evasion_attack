...
parser.add_argument("--transfer", default=False, required=False, action='store_true')
parser.set_defaults(transfer=False)
args = parser.parse_args()

num_hidden = 8
model_file_name = f"FashionMNIST_rnn_regressor_trial_{args.trial}.pt"
if args.transfer == True:
    print("Training transfer victim")
    num_hidden = 16
    model_file_name = "FashionMNIST_rnn_regressor_transfer.pt"
...
def main(model_func, train_func):
    ...
    model = model_func(input_dim, num_classes=len(LABELS_IN_USE), num_hidden=num_hidden)
    train_func(model)
    return model
