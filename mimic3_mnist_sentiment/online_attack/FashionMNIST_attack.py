...
def main(model_func, train_func):
    ...
    model = model_func(input_dim, num_classes=len(LABELS_IN_USE), num_hidden=8)
    return model
...
model.load_state_dict(torch.load("tmp/FashionMNIST_rnn_regressor.pt"))
...
if args.transfer_victim_path != None:
    ...
    eval_victim_model = LSTMRealTimeRegressor(input_dim=28, num_classes=len(LABELS_IN_USE), num_hidden=16)
...
for i, (data, target) in enumerate(test_loader):
    ...
    if i == 0 and args.save_result_image == True:
        ...
        plt.savefig("fashionmnist_advplot.jpg")
        plt.savefig("fashionmnist_advplot.pdf")
