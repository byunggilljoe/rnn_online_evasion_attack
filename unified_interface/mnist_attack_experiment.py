...
class MNISTAttackExperiment(AttackExperiment):
    ...
    def get_targeted_label(self, BATCH, true_label, t_start, NUM_WAVE=2):
        ...
        target_label_all = np.array([target_label for _ in range(BATCH)])
        if t_start > 0:
    ...
    def test_and_finish(self, param_list, batch_x_adv_list, criterion, eval_model, prefix, elapsed_time):
        ...
        with torch.no_grad():
            for params, batch_x_adv in zip(param_list, batch_x_adv_list):
                ...
                for t in rg:
                    ...
                    #STEP_TSAR_LIST.append((COUNT_FOOL/(COUNT_DECISION - COUNT_NAT_FOOL)).item())
                    if target_label != None:
      ...
      "mean_loss,", torch.mean(torch.tensor(MEAN_LOSS_LIST)).item(), "elapsed time,", elapsed_time)
