...
def get_loss_func(num_loss_step=1, gamma=1.0, max_attack=False):
    ...
    def loss_func(out, label, gamma_array=gamma_array, target_label=None):
        ...
        if target_label == None:
            ...
            # print(out.size(), num_out.size(), true_label.size(), target_label.size(), num_loss_step)
...
class MortalityAttackExperiment(AttackExperiment):
    ...
    def load_data_and_model(self):
        ...
        # mortality attack parameters should be scaled
        train_data = self.train_loader.dataset.tensors[0].numpy()
        self.feature_max = torch.tensor(np.percentile(np.concatenate(train_data, axis=0), 95)).cuda().float()
        self.feature_min = torch.tensor(np.percentile(np.concatenate(train_data, axis=0), 5)).cuda().float()
    ...
    def get_preparation(self, args, steering_net, test_generator, start_index):
        ...
        arg_dict["current_data"] = torch.clamp(arg_dict["current_data"], self.MIN_VALUE, self.MAX_VALUE)

        return batch_x, batch_y, criterion, arg_dict

    def get_targeted_label(self, BATCH, true_label, t_start, NUM_WAVE=2):
        NUM_STEP = 2*NUM_WAVE
        LEN_STEP = int(48/NUM_STEP) if NUM_STEP != 0 else 48
        target_label = []
        for i in range(48):
            target_label.append(int(i/LEN_STEP)%2)
    ...
    def test_and_finish(self, params_list, batch_x_adv_list, criterion, eval_model, prefix, elapsed_time):
        COUNT_FOOL = 0
        COUNT_DECISION = 0
        COUNT_NAT_FOOL = 0
        MAX_LOSS_LIST = []
        ...
        for params, batch_x_adv in zip(params_list, batch_x_adv_list):
            ...
            with torch.no_grad():
                ...
                #rg = range(self.args.t_start, self.args.t_end) if self.args.eval_last == False else [-1]
                ...
                for t in rg:
                    ...
                    if target_label != None:
                        target_match = adv_pred[:, t, :].max(dim=1)[1] == target_label[:, t]
                        COUNT_TARGET_MATCH += target_match.float().sum()
                
                if len(MAX_LOSS_LIST) == 1 and self.args.targeted == True:
                    # print("params['epsilon']:", params['epsilon'])
                    np.savez(f"../experiments/tmp_output/mortality_{prefix}_{params['epsilon'][0]:.02f}_visualization_data.npz", batch_x=batch_x.cpu().detach().numpy(), batch_y=batch_y.cpu().detach().numpy(),
