...
class AdvDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.type == 'train':
            ...
            img_address = img_address.replace(".png", ".jpg")
            #print(img_address)
...
class UdacityDataset(Dataset):
    ...
    def __init__(self, root_dir, hmb_list, transform=None, type_='train', downsample_factor=1.0):
        self.root_dir = root_dir
        self.hmb_list = hmb_list
        self.type = type_
        self.data = self.get_data()
        downsample_array = [i%downsample_factor == 0 for i in range(len(self.data))]
        self.data = self.data.iloc[downsample_array]
        print("Dataset steer angle max/min:", self.data["steering_angle"].max(), self.data["steering_angle"].min())
...
class UdacityDataset_LSTM(Dataset):
    
    def __init__(self, root_dir, hmb_list, transform=None, type_='train', num_frame_per_sample=10, downsample_factor=1.0):
        self.root_dir = root_dir
        self.hmb_list = hmb_list
        self.type = type_
        self.data = self.get_data()
        print("steer:", self.data["steering_angle"].max(), self.data["steering_angle"].min())
        self.transform = transform
        self.num_frame_per_sample = num_frame_per_sample
    ...
    def get_data(self):
        ...
        if self.type == 'train':
            for hmb in self.hmb_list:
                df = pd.read_csv(os.path.dirname(os.path.realpath(__file__)) + "/"+hmb + '.csv')
                data_info.append(df)
            return pd.concat(data_info)
            
        elif self.type == 'test':
            df = pd.read_csv(os.path.dirname(os.path.realpath(__file__)) + "/" + 'ch2_final_eval.csv')
            df['frame_id'] = df['frame_id'].apply(str)
            return df

    def __len__(self):
        # return len(self.images)
        return (len(self.data)-self.num_frame_per_sample)//self.num_frame_per_sample

    def __getitem__(self, sample_idx):
        frame_start_index = sample_idx * self.num_frame_per_sample
        sample_list = []
        for i in range(self.num_frame_per_sample):
            sample_list.append(self.get_one_frame(frame_start_index + i))
        image_sequence_list = torch.cat([one_frame['image'].unsqueeze(0) for one_frame in sample_list])
        steer_sequence_list = torch.cat([one_frame['steer'] for one_frame in sample_list])

        return image_sequence_list, steer_sequence_list
...
if __name__ == "__main__":
    ...
    composed = transforms.Compose([Rescale((128, 128)),  RandRotation(), Preprocess()])
